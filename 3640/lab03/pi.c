...
int main ()
{
	...
	omp_set_num_threads(1); // allocate # of threads
	...
	#pragma omp parallel // give each thread their own section of curve to calculate & sum at end 
	{
		...
		double x, sum = 0.0;
		if (cID == 0) {
			for (i = 1; i <= num_steps; i++){ // goes through each rectangle on the curve and sums them
				x = (i + 0.5) * step;
				sum += 4.0 / (1.0 + x * x);
			}
		...
		/*
		if (cID == 1) {
			for (i = num_steps / 2; i <= num_steps; i++){ // goes through each rectangle on the curve and sums them
				x = (i - 0.5) * step;
				sum = sum + 4.0 / (1.0 + x * x);
			}
		...
		if (cID == 2) {
			for (i = num_steps * 2 / 3; i <= num_steps; i++){ // goes through each rectangle on the curve and sums them
				x = (i - 0.5) * step;
				sum = sum + 4.0 / (1.0 + x * x);
			}
