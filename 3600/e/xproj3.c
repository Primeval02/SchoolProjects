...
int main(int argc, char *argv[], char *envp[])
{
    ...
    x11_init_xwindows();
    //
    sem_init(&hand, 0, 1); //initialize semaphore to 0 
    pthread_t th1, th2;
    pthread_create(&th1, NULL, movement, (void *)0);
    pthread_create(&th2, NULL, movement, (void *)1);
    //
...
void* movement(void* arg)
{
    ...
    while(1) {
        ...
        if (balls[i].state == 0) {
            if (balls[i].pos[0] < catching_hand_pos[0])
                balls[i].pos[0] = catching_hand_pos[0];
...
void render(void)
{
    ...
    y += 12;
    char str[64];
    sprintf(str, "ball_state: %i", ball.state);
    drawText(12, y, str);
    //
    //draw the hands
    int xp, yp;
