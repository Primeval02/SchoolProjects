...
int main(int argc, char *argv[])
{
    ...
    int outf = open(argv[2], O_WRONLY|O_CREAT|O_TRUNC, 0666); /* Output file */
    ...
    char *dest = mmap(NULL, length + offset - pa_offset, PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
	if (dest == MAP_FAILED)
		perror("mmap");
    ...
    memcpy(dest, addr + offset, length);
	...
	//write(STDOUT_FILENO, "\n", 1);
    write(outf, dest, length + offset - pa_offset);
    write(outf, "\n", 2);
    printf("%li bytes written to file.\n", length);
	/*if (s != length) {
		if (s == -1)
			perror("write");
