	...
	if (useArcBall) {
		...
		if (input.current.keyStates[GLFW_KEY_LEFT_ALT]) {
			if (input.current.mouseStates[GLFW_MOUSE_BUTTON_LEFT]) {
				...
				float dist = distance - distRange.x;
				...
				float s = glm::sign(deltaMouse.y);

				// Calculate zoom amount proportional to mouse input and current zoom distance
				float zoomAmount = (s * distRange.x) + (deltaMouse.y * logd);

				zoomAmount = deltaMouse.y / (0.5f * height);

				distance = glm::clamp(distance + zoomAmount, distRange.x, distRange.y);
			}
