//Name: Bradley Tate
//
//Most recent OpenGL version: 4.2
...
void renderCelestialBody(const CelestialBody & body, float* pixels, int stride, int width, int height) {
	...
	mat4 projection = glm::perspective(fovy, aspectRatio, 1.0f, 1000.0f);
...
// TODO: edit the initialization values for the sun and planet as needed
void init() {
	...
	planetBody.transform.rotation = vec4(0, 0, 1, 0);
	planetBody.transform.scale = vec3(4);
	planetBody.color = vec3(0.1, 0.1, 0.8);
	planetBody.orbitalRotation.z = 1;
	...
	moonBody.name = "Moon";
	moonBody.transform.translation = vec3(30, 0, 0);
	moonBody.transform.rotation = vec4(0, 0, 1, 0);
	moonBody.transform.scale = vec3(1.25);
	moonBody.color = vec3(0.5, 0.5, 0.5);
	moonBody.useModelMatrix = true;
	moonBody.orbitalRotation.z = 1;
...
// TODO: update the planet's modelMatrix so it orbits the sun and spins on its axis.
void Lab05::render(s_ptr<Texture> screen) {
	...
	// Use either of these to help change rotation over time.
	...
	if (celestialBodies.size() > 1) {
		...
		// planet stuff (Main assignment)
		{
			mat4 T = glm::translate(planet.transform.translation);

			// this creates an axis-angle rotation matrix using from the rotation vec4.
			// xyz is the axis, w is the angle. 
			// To change rotation over time, you'll want to modify planet.transform.rotation.w every frame
			planet.transform.rotation.w += deltaTime;
			...
			if (planet.orbitalRotation.w != 0) {
				orbit = glm::rotate(planet.orbitalRotation.w, vec3(planet.orbitalRotation));
			}

			// Right now this just applies the planet's Transform2D data (settable from the UI) to the object.
			// Figure out the transformations needed to make the planet orbit the sun and spin on its axis
			planet.modelMatrix = orbit * T * R * S;
		}

		// moon stuff (Extra credit)
		if (celestialBodies.size() > 2) {
			CelestialBody& moon = celestialBodies[2];
			mat4 T = glm::translate(moon.transform.translation);

			moon.transform.rotation.w += deltaTime;
			...
			if (moon.orbitalRotation.w != 0) {
				orbit = glm::rotate(moon.orbitalRotation.w, vec3(moon.orbitalRotation));
			}

			moon.modelMatrix = orbit * T * R * S;
		}
	}
