...
void RaySphereHit(const RaySceneObject& object, const Ray& ray, HitRecord& hr) {
	...
	float b = glm::dot(glm::vec3(2 * ray.R.x, 2 * ray.R.y, 2 * ray.R.z), (ray.p0 - C));
	float c = glm::dot((ray.p0 - C), (ray.p0 - C)) - (r * r);
	float numCheck = b * b - 4 * a * c;
	float t, t1, t2 = 0.0;
		...
		for (int y = 0; y < height; y++) {
			...
			if (hr.hit) {
				if (colorMode == +RTColorMode::Default) {
					...
					/* Not working, bad type mismatching, come back to after rays work properly

					vec3 norm = glm::normalize(hr.normal);
					vec3 lightDir = glm::normalize(-light.lightDirection);
					vec3 halfway = glm::normalize(lightDir + ray.R);
					vec3 diff = std::max(glm::dot(norm, lightDir), 0.0f) * hr.object->material.Kd;
					vec3 spec = glm::vec3(pow(std::max(0.0f, glm::dot(halfway, norm)), hr.object->material.reflective));
					color = diff + spec;
					*/	
				}
