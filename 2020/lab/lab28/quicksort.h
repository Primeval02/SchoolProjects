...
#include "vt100ansi.h"     // For ANSI colors

#define SLEEP_USEC 2500 // Microseconds to pause between output
...
int split(T a[], int first, int last)
  ...
  while(i < j)
    ...
    while(a[j] > pivot)
      {
      j--;  // search backwards for a[j] <= pivot
      }
    while(i < j && a[i] <= pivot)
      {
      i++;  // search forwards a[i] > pivot
      }
  ...
  //
  // Also, if we had moved the pivot to the last position (instead of the
  // first position) we would swap with i instead of j.
  swap(a[first], a[j]);
...
void quicksort(T a[], int first, int last)
  ...
  if(size < THRESHOLD)
    ...
    // Choose a pivot and partition into larger ('right') and smaller ('left')
    int p = split(a, first, last);
    quicksort(a, first, p - 1);  // sort the smaller elements ('left')
    quicksort(a, p + 1, last);   // sort the larger elements ('right')
    }
...
// Array is going to start at first instead of 1 and end at last instead of n,
// so must adjust the loops accordingly from the insertion sort lab.
template <class T>
void insertionsort(T a[], int first, int last)
  {
  T tmp;
  ...
  if(size <= 1)
    {
    return;  // 0 or 1 element list is already sorted
    }
  ...
  for(int p = first + 1; p <= last; p++)
    ...
    for(j = p; j > first; j--)
      ...
      if(tmp >= a[j - 1])
        {
        break;  // The hole has been found, end the loop
        }
