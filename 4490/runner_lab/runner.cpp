...
void init_opengl(void)
{
    ...
    //floor
	glGenTextures(1, &g.flrid);
    glBindTexture(GL_TEXTURE_2D, g.flrid);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
	...
	for (int i=0; i<sprite.height; i++) {
		for (int j=0; j<sprite.width; j++) {
			...
			data2[offset2+0] = sprite.data[offset+1];
...
void render()
{
    ...
    /*
	float tx1 = 0.0f + (float)((g.frameno-1) % 9) * 0.111f;
	float tx2 = tx1 + 0.111f;
	float ty1 = 0.0f + (float)((g.frameno-1) / 1) * 1.0f;
	float ty2 = ty1 + 1.0;
	float w = g.xres/4;
	float h = g.yres/2;
    */
	float tx1 = 0.0f + (float)((g.frameno-1) % 8) * 0.125f;
