...
void crossProduct(Vec v1, Vec v2, Vec v3) {
	...
	if (e->type == KeyPress) {
		...
		switch(key) {
            ...
            case XK_2:
                glDisable(GL_FOG);
                break;
            case XK_r: // record and calls screenshot
                screenShot();
                break;
            case XK_t: // finalize and convert all saved imgs into a gif
                system("convert -loop 0 -coalesce -layers OptimizeFrame -delay 20 ./images/img*.jpg abc.gif"); 
                break;
            case XK_w:  // Move forward
                for (int i = 0; i < 3; i++) {
                    g.cameraPosition[i] += g.cameraDir[i] * 0.15;
                }
                break;
            case XK_s: // Move backward
                for (int i = 0; i < 3; i++) {
                    g.cameraPosition[i] -= g.cameraDir[i] * 0.15;
                }
                break;
            case XK_a: //move left along an arbitrary axis
                { 
                    Vec left;
                    Vec up = {0.0, 1.0, 0.0};
                    crossProduct(g.cameraDir, up, left);
                    for (int i = 0; i < 3; i++) {
                        g.cameraPosition[i] -= left[i] * 0.15;
                    }
                }
                break;
            case XK_d: //move right along arbitrary axis
                {
                    Vec left;
                    Vec up = {0.0, 1.0, 0.0};
                    crossProduct(g.cameraDir, up, left);
                ...
                break;
			case XK_Up: //tilts the camera up (didn't like inverted)
                {
                    Matrix m;
                    Vec up = {0.0, 1.0, 0.0};
                    Vec left;
				...
				break;
			case XK_Down: //tilts the camera down (didn't like inverted)
	            {
                    Matrix m;
                    Vec up = {0.0, 1.0, 0.0};
                    Vec left;
			...
			case XK_Left: //rotate left
				...
				break;
			case XK_f: //old forward & back keys
                for (int i = 0; i < 3; i++) {
                    g.cameraPosition[i] += g.cameraDir[i] * 0.1;
                }
				break;
			case XK_b:
                for (int i = 0; i < 3; i++) {
				...
				break;
			case XK_Escape: //terminate early
				return 1;
		}
	}
	return 0;
...
void Global::render()
{
	...
	//
    //Draw cube
    static float angle = 0.0;  //Move to physics
	glPushMatrix();
    ...
    ggprint8b(&r, 16, 0x00887766, "move: WASD");
    ggprint8b(&r, 16, 0x00887766, "camera: Arrow Keys");
    ggprint8b(&r, 16, 0x00887766, "enable/disable fog: 1/2");
    ggprint8b(&r, 16, 0x00887766, "screenshot: R");
    ggprint8b(&r, 16, 0x00887766, "covert all screenshots to gif: T");
	glPopAttrib();
}
