...
//} img("/home/stu/yours/4490/8/flowers.ppm");
} img("/home/fac/gordon/4490/x8/flowers.jpg"),
  sprite("/home/fac/gordon/4490/x8/bees.png");
...
public:
	...
	Bee() {
		w = h = 4.0;
		pos[0] = 1.0;
		pos[1] = 200.0 + (Flt)(rand() % 10);
		vel[0] = 4.0 + (Flt)(rand() % 2);
		vel[1] = 0.0;
		//printf("pos[1]: %lf\n", pos[1]); fflush(stdout);
...
void X11_wrapper::reshape_window(int width, int height)
{
	...
	for (int i=0; i<g.nbees; i++) {
		g.bees[i].set_dimensions(g.xres, g.yres);
	}
}
...
void physics()
{
	...
	//movement
	for (int i=0; i<2; i++) {
		g.bees[i].pos[0] += g.bees[i].vel[0] * ((double)g.xres / g.sxres);
		g.bees[i].pos[1] += g.bees[i].vel[1] * ((double)g.yres / g.syres);
		//boundry test
		if (g.bees[i].pos[0] >= g.xres) {
			g.bees[i].pos[0] = g.xres;
			g.bees[i].vel[0] = 0.0;
		}
		if (g.bees[i].pos[0] <= 0) {
			g.bees[i].pos[0] = 0;
		...
		if (g.bees[i].pos[1] >= g.yres) {
			g.bees[i].pos[1] = g.yres;
			g.bees[i].vel[1] = 0.0;
		}
		if (g.bees[i].pos[1] <= 0) {
			g.bees[i].pos[1] = 0;
		...
		Flt dx = cx - g.bees[i].pos[0];
		Flt dy = cy - g.bees[i].pos[1];
		Flt dist = (dx*dx + dy*dy);
		if (dist < 0.01)
			dist = 0.01; //clamp
		//change in velocity based on a force (gravity)
		g.bees[i].vel[0] += (dx / dist) * g.gravity;
		g.bees[i].vel[1] += (dy / dist) * g.gravity;
		g.bees[i].vel[0] += ((Flt)rand() / (Flt)RAND_MAX) * 0.5 - 0.25;
		g.bees[i].vel[1] += ((Flt)rand() / (Flt)RAND_MAX) * 0.5 - 0.25;
	}
}
...
void render()
{
	...
	r.center = 0;
	int score = 25;
	ggprint8b(&r, 0, 0x00ffffff, "Gordon score: %i", score);
	//
	//Draw bees.
	for (int i=0; i<2; i++) {
		glPushMatrix();
		glColor3ub(255, 255, 255);
		glTranslatef(g.bees[i].pos[0], g.bees[i].pos[1], 0.0f);
		//set alpha test
		...
		float ty2 = ty1 + 0.2;
		if (g.bees[i].vel[0] > 0.0) {
			float tmp = tx1;
			tx1 = tx2;
			tx2 = tmp;
		}
		glBegin(GL_QUADS);
			glTexCoord2f(tx1, ty2); glVertex2f(-g.bees[i].w, -g.bees[i].h);
			glTexCoord2f(tx1, ty1); glVertex2f(-g.bees[i].w,  g.bees[i].h);
			glTexCoord2f(tx2, ty1); glVertex2f( g.bees[i].w,  g.bees[i].h);
			glTexCoord2f(tx2, ty2); glVertex2f( g.bees[i].w, -g.bees[i].h);
		glEnd();
	    glBindTexture(GL_TEXTURE_2D, 0);
